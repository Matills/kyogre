<template>
    <div class="min-h-screen flex flex-col items-start justify-start bg-secondary text-white pt-4 pl-4">
      <div class="flex items-center mb-4">
        ...
        <input type="radio" v-model="action" value="purchase" class="text-primary mr-2 mt-1" />
        <h1 class="text-primary text-2xl font-bold mx-2">/</h1>
        <h1 class="text-primary text-2xl font-bold mr-2">Vender</h1>
        <input type="radio" v-model="action" value="sale" class="text-primary mt-1" />
      </div>
  
      <div class="mb-4 w-full max-w-md">
        <label for="monto" class="block text-white mb-2">De {{ selectedFiat === 'ARS' ? 'Pesos Argentinos' : getCoinName(selectedFiat) }}</label>
        <div class="flex">
          <input
            id="monto"
          ...
          />
          <select v-model="selectedFiat" @change="updateConversion" class="p-2 bg-white text-black rounded-r-lg">
      ...
      <div class="mb-4 w-full max-w-md">
        <label for="conversion" class="block text-white mb-2">A {{ getCoinName(selectedCrypto) }}</label>
        <div class="flex">
          <input
            id="conversion"
      ...
      <p class="mb-6 text-white">
        Tasa de cambio: 1 {{ selectedCrypto }} = {{ exchangeRate }} {{ selectedFiat }}
      </p>
  
      <button
        @click="confirmTransaction"
        class="bg-accent text-white py-2 px-4 rounded-lg hover:bg-green-600 transition-colors"
      >
  ...
  const validateAmount = () => {
    ...
    if (amount.value > 0 && selectedFiat && selectedCrypto) {
      updateConversion();
    }
  }
  ...
  const updateConversion = async () => {
    if (amount.value > 0) {
      try {
        const criptoData = await getCripto(selectedCrypto.value, selectedFiat.value)
  ...
  const confirmTransaction = async () => {
    ...
    try {
      ...
      alertTitle.value = 'Transacción Exitosa';
